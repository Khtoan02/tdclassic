...
<main id="main" class="site-main products-page antialiased selection:bg-gold selection:text-black" style="background-color: #050505;">
    
    <!-- PAGE HERO -->
    <section class="relative pt-24 md:pt-32 pb-16 md:pb-24 bg-void overflow-hidden">
        <div class="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-[#151515] to-transparent opacity-30"></div>
        <div class="container mx-auto px-4 md:px-6 lg:px-12 relative z-10 text-center">
            <span class="font-sans text-[#C5A059] text-xs tracking-[0.3em] uppercase block mb-4 md:mb-6">Bộ sưu tập 2025</span>
            <h1 class="font-serif text-4xl md:text-6xl lg:text-7xl text-white mb-4 md:mb-6" style="font-family: 'Cinzel', serif;">KHO TÀNG ÂM THANH</h1>
            <p class="font-sans text-gray-400 text-sm md:text-base font-light leading-relaxed max-w-2xl mx-auto" style="font-family: 'Manrope', sans-serif;">
                Khám phá hệ sinh thái thiết bị âm thanh chuyên nghiệp. Được thiết kế để đáp ứng mọi quy mô dự án từ phòng hát gia đình đến sân khấu biểu diễn hàng ngàn khán giả.
            </p>
        </div>
    </section>

    <!-- STICKY FILTER BAR -->
    <div class="sticky top-0 z-30 bg-[#050505]/80 transition-all duration-300" style="backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-bottom: 1px solid rgba(255,255,255,0.05);">
        <div class="container mx-auto px-6 md:px-12">
            <div class="flex items-center justify-between h-16">
                <!-- Mobile Scrollable List -->
                <div class="flex overflow-x-auto gap-8 w-full md:justify-center items-center" style="-ms-overflow-style: none; scrollbar-width: none;">
                    <?php
                    ...
                    ?>
                    <a href="#all" class="category-filter-link active font-sans text-xs font-bold uppercase tracking-widest whitespace-nowrap border-b-2 border-[#C5A059] pb-4 mt-4 hover:text-white transition-colors" style="color: #C5A059;" data-category="all">Tất cả</a>
                    
                    <?php
                    if ($product_categories && !is_wp_error($product_categories)) :
                        foreach ($product_categories as $category) :
                    ?>
                        <a href="#<?php echo $category->slug; ?>" class="category-filter-link text-gray-500 hover:text-white font-sans text-xs font-bold uppercase tracking-widest whitespace-nowrap pb-4 mt-4 transition-colors" data-category="<?php echo $category->slug; ?>"><?php echo $category->name; ?></a>
                    <?php
                        endforeach;
                    endif;
                    ?>
    ...
    <section id="<?php echo $category->slug; ?>" class="py-16 md:py-24 <?php echo $bg_class; ?> border-b border-white/5" data-category="<?php echo $category->slug; ?>">
        <div class="container mx-auto px-4 md:px-6 lg:px-12">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-12 gap-4 md:gap-6">
                <div>
                    <h2 class="font-serif text-3xl text-white" style="font-family: 'Cinzel', serif;"><?php echo $category->name; ?></h2>
                    <p class="font-sans text-gray-500 text-xs mt-2 uppercase tracking-widest" style="font-family: 'Manrope', sans-serif;">
            ...
            <?php if ($spotlight) : ?>
            <!-- Spotlight Item -->
            <div class="mb-8 md:mb-12 product-card group cursor-pointer relative bg-[#151515] border border-white/5 overflow-hidden h-[350px] md:h-[450px] lg:h-[500px]">
                <a href="<?php echo $spotlight['url']; ?>" class="block w-full h-full">
                    <div class="img-container w-full h-full">
                        <?php if ($spotlight['thumbnail']) : ?>
                            <img src="<?php echo $spotlight['thumbnail']; ?>" class="w-full h-full object-cover transition-transform duration-[800ms] ease-[cubic-bezier(0.2,1,0.3,1)] group-hover:scale-[1.08]" alt="<?php echo $spotlight['title']; ?>" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\'w-full h-full bg-gradient-to-br from-[#1E1E1E] to-[#0A0A0A] flex items-center justify-center\'><svg class=\'w-24 h-24 text-[#C5A059] opacity-30\' fill=\'none\' stroke=\'currentColor\' viewBox=\'0 0 24 24\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'1.5\' d=\'M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3\'></path></svg></div>';">
                        <?php else : ?>
                            <div class="w-full h-full bg-gradient-to-br from-[#1E1E1E] to-[#0A0A0A] flex items-center justify-center">
                                <svg class="w-24 h-24 text-[#C5A059] opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3"></path>
                                </svg>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div class="overlay absolute inset-0 flex flex-col justify-end p-6 md:p-10 lg:p-12 opacity-0 group-hover:opacity-100 transition-opacity duration-400" style="background: linear-gradient(to top, rgba(0,0,0,0.9) 0%, transparent 100%);">
                        <span class="bg-[#C5A059] text-black text-[10px] font-bold uppercase tracking-wider px-3 py-1 inline-block w-fit mb-3">Signature Series</span>
                        <h3 class="font-serif text-2xl md:text-4xl lg:text-5xl text-white mb-2" style="font-family: 'Cinzel', serif;"><?php echo $spotlight['title']; ?></h3>
                        <p class="font-sans text-gray-300 text-sm mb-6 max-w-lg hidden md:block" style="font-family: 'Manrope', sans-serif;">Sản phẩm chuyên nghiệp hàng đầu</p>
                        <div class="flex items-center gap-4">
                            <span class="bg-white text-black px-6 py-3 font-sans text-xs font-bold uppercase tracking-widest hover:bg-[#C5A059] transition-colors">Xem chi tiết</span>
                        </div>
                    </div>
                </a>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                <a href="<?php echo $product['url']; ?>" class="product-card group cursor-pointer bg-[#151515] border border-white/5 p-4 hover:border-[#C5A059]/30 transition-all">
                    <div class="img-container aspect-square bg-[#1E1E1E] overflow-hidden mb-4 relative">
                        <?php if ($product['thumbnail']) : ?>
                            <img src="<?php echo $product['thumbnail']; ?>" class="w-full h-full object-cover transition-transform duration-[800ms] ease-[cubic-bezier(0.2,1,0.3,1)] group-hover:scale-[1.08]" alt="<?php echo $product['title']; ?>" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\'w-full h-full bg-gradient-to-br from-[#1E1E1E] to-[#0A0A0A] flex items-center justify-center\'><svg class=\'w-16 h-16 text-[#C5A059] opacity-30\' fill=\'none\' stroke=\'currentColor\' viewBox=\'0 0 24 24\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'1.5\' d=\'M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3\'></path></svg></div>';">
                        <?php else : ?>
                            <div class="w-full h-full bg-gradient-to-br from-[#1E1E1E] to-[#0A0A0A] flex items-center justify-center">
                                <svg class="w-16 h-16 text-[#C5A059] opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3"></path>
                                </svg>
                            </div>
                        <?php endif; ?>
                    </div>
                    <h4 class="font-serif text-white text-sm md:text-base group-hover:text-[#C5A059] transition-colors" style="font-family: 'Cinzel', serif;"><?php echo $product['title']; ?></h4>
                    <p class="font-sans text-gray-500 text-[10px] uppercase mt-1" style="font-family: 'Manrope', sans-serif;">Chuyên nghiệp</p>
                </a>
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
            ...
            <div class="text-center mt-12">
                <a href="<?php echo get_term_link($category); ?>" class="btn-load-more group inline-flex flex-col items-center gap-2 text-gray-400 hover:text-[#C5A059] transition-colors">
                    <span class="font-sans text-xs font-bold uppercase tracking-widest">Xem tất cả <?php echo $products_query->found_posts; ?> sản phẩm</span>
    ...
    <section class="py-16 md:py-20 bg-[#151515] relative overflow-hidden border-y border-white/5">
        <div class="absolute inset-0 opacity-20">
            <img src="https://images.unsplash.com/photo-1470225620780-dba8ba36b745?q=80&w=1600&auto=format&fit=crop" class="w-full h-full object-cover grayscale">
        </div>
        <div class="container mx-auto px-6 text-center relative z-10">
            <h2 class="font-serif text-2xl md:text-3xl lg:text-4xl text-white mb-3 md:mb-4" style="font-family: 'Cinzel', serif;">Dự Án Của Bạn Cần Gì?</h2>
            <p class="font-sans text-gray-400 text-xs md:text-sm max-w-xl mx-auto mb-6 md:mb-8" style="font-family: 'Manrope', sans-serif;">
                Hệ thống âm thanh Bar, Lounge, Karaoke hay Hội trường? Chúng tôi có giải pháp tối ưu cho từng không gian.
            </p>
            <a href="<?php echo home_url('/lien-he'); ?>" class="inline-block bg-transparent border border-[#C5A059] text-[#C5A059] hover:bg-[#C5A059] hover:text-black font-sans text-xs font-bold uppercase tracking-widest px-8 py-3 transition-all">
    ...
    <section class="py-12 md:py-16 lg:py-20 bg-void border-t border-white/5 text-gray-600 font-sans text-[10px] leading-relaxed" style="font-family: 'Manrope', sans-serif;">
