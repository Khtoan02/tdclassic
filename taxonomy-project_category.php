<?php
/**
 * The template for displaying project category archive pages
 */

get_header(); ?>

<main id="main" class="site-main">
    <div class="container py-5">
        <div class="row">
            <div class="col-lg-12">
                <header class="page-header mb-5">
                    <h1 class="page-title">
                        <i class="fas fa-briefcase me-2"></i>
                        Danh mục dự án: <?php single_term_title(); ?>
                    </h1>
                    <?php if (term_description()) : ?>
                        <div class="page-description lead text-muted">
                            <?php echo term_description(); ?>
                        </div>
                    <?php endif; ?>
                    
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="<?php echo home_url('/'); ?>">Trang chủ</a></li>
                            <li class="breadcrumb-item"><a href="<?php echo home_url('/du-an'); ?>">Dự án</a></li>
                            <li class="breadcrumb-item active" aria-current="page"><?php single_term_title(); ?></li>
                        </ol>
                    </nav>
                </header>

                <!-- Filter Section -->
                <div class="filter-section mb-4">
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <h5 class="mb-0">
                                <?php 
                                $term = get_queried_object();
                                $count = $term->count;
                                echo $count;
                                ?> dự án
                            </h5>
                        </div>
                        <div class="col-md-6">
                            <div class="d-flex justify-content-md-end">
                                <select class="form-select" style="width: auto;" onchange="sortProjects(this.value)">
                                    <option value="newest">Mới nhất</option>
                                    <option value="oldest">Cũ nhất</option>
                                    <option value="name-asc">Tên A-Z</option>
                                    <option value="name-desc">Tên Z-A</option>
                                </select>
                            </div>
                        </div>
                    </div>
                </div>

                <?php if (have_posts()) : ?>
                    <div class="row g-4">
                        <?php while (have_posts()) : the_post(); ?>
                            <div class="col-lg-6 col-md-12">
                                <article class="project-post">
                                    <div class="card h-100">
                                        <?php if (has_post_thumbnail()) : ?>
                                            <img src="<?php the_post_thumbnail_url('blog-thumb'); ?>" class="card-img-top" alt="<?php the_title(); ?>">
                                        <?php else : ?>
                                            <div class="card-img-top bg-light d-flex align-items-center justify-content-center" style="height: 250px;">
                                                <span class="text-muted">Project Image</span>
                                            </div>
                                        <?php endif; ?>
                                        
                                        <div class="card-body d-flex flex-column">
                                            <div class="project-meta mb-3">
                                                <small class="text-muted">
                                                    <i class="fas fa-calendar-alt me-1"></i>
                                                    <?php echo get_the_date(); ?>
                                                    <span class="mx-2">|</span>
                                                    <i class="fas fa-folder me-1"></i>
                                                    <?php
                                                    $terms = get_the_terms(get_the_ID(), 'project_category');
                                                    if ($terms) {
                                                        $term_names = array();
                                                        foreach ($terms as $term) {
                                                            $term_names[] = '<a href="' . get_term_link($term) . '">' . $term->name . '</a>';
                                                        }
                                                        echo implode(', ', $term_names);
                                                    }
                                                    ?>
                                                    <span class="mx-2">|</span>
                                                    <span class="badge bg-success">Hoàn thành</span>
                                                </small>
                                            </div>
                                            
                                            <h5 class="card-title">
                                                <a href="<?php the_permalink(); ?>" class="text-decoration-none text-dark">
                                                    <?php the_title(); ?>
                                                </a>
                                            </h5>
                                            
                                            <p class="card-text flex-grow-1"><?php the_excerpt(); ?></p>
                                            
                                            <div class="mt-auto">
                                                <a href="<?php the_permalink(); ?>" class="btn btn-outline-primary">
                                                    Xem chi tiết
                                                </a>
                                                
                                                <div class="project-tech mt-3">
                                                    <span class="badge bg-secondary me-1">PHP</span>
                                                    <span class="badge bg-secondary me-1">WordPress</span>
                                                    <span class="badge bg-secondary me-1">Bootstrap</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </article>
                            </div>
                        <?php endwhile; ?>
                    </div>

                    <!-- Pagination -->
                    <div class="row mt-5">
                        <div class="col-12">
                            <?php tdclassic_pagination(); ?>
                        </div>
                    </div>

                <?php else : ?>
                    <div class="alert alert-info text-center" role="alert">
                        <h4>Không có dự án nào trong danh mục này</h4>
                        <p class="mb-0">Hiện tại danh mục "<?php single_term_title(); ?>" chưa có dự án nào.</p>
                        <a href="<?php echo home_url('/du-an'); ?>" class="btn btn-primary mt-3">Xem tất cả dự án</a>
                    </div>
                <?php endif; ?>

                <!-- Project Stats -->
                <div class="project-stats mt-5">
                    <div class="row">
                        <div class="col-md-3">
                            <div class="card text-center bg-light">
                                <div class="card-body">
                                    <h3 class="text-primary mb-2"><?php echo $count; ?></h3>
                                    <p class="mb-0">Dự án hoàn thành</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card text-center bg-light">
                                <div class="card-body">
                                    <h3 class="text-success mb-2">100%</h3>
                                    <p class="mb-0">Tỷ lệ thành công</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card text-center bg-light">
                                <div class="card-body">
                                    <h3 class="text-warning mb-2">3</h3>
                                    <p class="mb-0">Tháng trung bình</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card text-center bg-light">
                                <div class="card-body">
                                    <h3 class="text-info mb-2">24/7</h3>
                                    <p class="mb-0">Hỗ trợ</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Other Categories -->
                <div class="other-categories mt-5">
                    <h4 class="mb-4">Danh mục dự án khác</h4>
                    <div class="row g-3">
                        <?php
                        $current_term = get_queried_object();
                        $project_categories = get_terms(array(
                            'taxonomy' => 'project_category',
                            'hide_empty' => true,
                            'exclude' => $current_term->term_id
                        ));
                        
                        if ($project_categories) :
                            foreach ($project_categories as $category) :
                        ?>
                            <div class="col-md-3">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <h6 class="card-title"><?php echo $category->name; ?></h6>
                                        <p class="card-text text-muted"><?php echo $category->count; ?> dự án</p>
                                        <a href="<?php echo get_term_link($category); ?>" class="btn btn-outline-primary btn-sm">
                                            Xem danh mục
                                        </a>
                                    </div>
                                </div>
                            </div>
                        <?php
                            endforeach;
                        endif;
                        ?>
                    </div>
                </div>

                <!-- Call to Action -->
                <div class="cta-section mt-5">
                    <div class="card bg-dark text-white">
                        <div class="card-body text-center py-5">
                            <h3 class="card-title">Bạn có dự án tương tự?</h3>
                            <p class="card-text lead">Hãy liên hệ với chúng tôi để được tư vấn miễn phí về dự án của bạn</p>
                            <a href="<?php echo home_url('/lien-he'); ?>" class="btn btn-light btn-lg">
                                Liên hệ ngay
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>

<script>
function sortProjects(sortBy) {
    const url = new URL(window.location);
    url.searchParams.set('orderby', sortBy);
    window.location.href = url.toString();
}
</script>

<?php get_footer(); ?> 